import { initializeApp } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-app.js";
        ...
        const ELO_RELEASE_NOTES = [
            ...
            '🐾 PET V2: coleção compartilhada, Pet ativo, inventário de ovos e incubadora independente.',
            '☄️ Cometas do Celestial agora apontam o rastro corretamente na direção oposta ao movimento.',
            '🐾 Central dos Pets corrigida: modal sempre acima da camada cinza, fundo bloqueado e fechamento previsível.',
            '✨ UX dos Pets refinada: Pet sai do caminho durante modais e a Central ganhou fechamento visível.'
        ];
        ...
        const ELO_ANDROID_VERSION = isNativeApp ? { versionName:'0.9.0', versionCode:34 } : { versionName:'0.0.0-web', versionCode:0 };
...
window.checkInToday = async (buttonEl = null) => {
        ...
        const renderThemeFx = (id, customCfg = null) => {
            ...
            const particles=Array.from({length:count},(_,i)=>`<i class="elo-space-particle" style="--i:${i};--x:${(i*37)%101};--y:${(i*53)%97};--d:${7+(i%8)*1.25}s;--delay:${-(i%11)*1.05}s;--size:${2+(i%4)}px"></i>`).join('');
            const comets=effect==='blackhole'?Array.from({length:5},(_,i)=>`<b class="elo-space-comet comet-${i+1}" style="--ci:${i};--cd:${16+i*4}s;--cDelay:${-i*6.4}s"><em></em></b>`).join(''):'';
            fx.innerHTML=(effect==='blackhole'?`<button type="button" class="elo-black-hole" onclick="reactCelestialBlackHole(event)" aria-label="Interagir com o buraco negro"><span class="elo-bh-lensing lens-back"></span><span class="elo-bh-accretion-glow"></span><span class="elo-bh-disk disk-back"></span><span class="elo-black-hole-core"><i></i></span><span class="elo-bh-photon-ring"></span><span class="elo-bh-disk disk-front"></span><span class="elo-bh-jet jet-a"></span><span class="elo-bh-jet jet-b"></span><span class="elo-bh-lensing lens-front"></span><span class="elo-black-hole-hint">toque para alterar a gravidade</span></button>`:'')+particles+comets;
        ...
        const swallowBlackHoleParticle = (particle, hole, delay=0) => {
            ...
            const dx=hx-px, dy=hy-py, angle=Math.atan2(dy,dx)*180/Math.PI, dist=Math.hypot(dx,dy), comet=particle.classList?.contains('elo-space-comet');
            const anim=particle.animate([
                {transform:'translate3d(0,0,0) rotate(0deg) scale(1)',opacity:comet?.9:.72,filter:'blur(0)',offset:0},
                {transform:`translate3d(${dx*.20}px,${dy*.18}px,0) rotate(${angle*.18}deg) scale(1.05)`,opacity:1,offset:.28},
                {transform:`translate3d(${dx*.48}px,${dy*.45}px,0) rotate(${angle*.55}deg) scaleX(${comet?1.9:2.4}) scaleY(.62)`,opacity:1,offset:.56},
                {transform:`translate3d(${dx*.78}px,${dy*.76}px,0) rotate(${angle}deg) scaleX(${comet?5.5:8.5}) scaleY(.12)`,opacity:.96,filter:'blur(.35px)',offset:.82},
                {transform:`translate3d(${dx}px,${dy}px,0) rotate(${angle}deg) scaleX(${comet?9:12}) scaleY(.018)`,opacity:0,filter:'blur(1.4px)',offset:1}
            ],{duration:(comet?1900:1250)+Math.min(1400,dist*1.45),delay,easing:'cubic-bezier(.12,.48,.08,1)',fill:'forwards'});
        ...
        window.reactCelestialBlackHole = event => {
            ...
            const particles=[...fx.querySelectorAll(':scope > .elo-space-particle')], comets=[...fx.querySelectorAll(':scope > .elo-space-comet')];
            ...
            chosen.forEach((p,i)=>swallowBlackHoleParticle(p,hole,Math.floor(i/3)*190+(i%3)*70));
            comets.slice(0,2).forEach((c,i)=>swallowBlackHoleParticle(c,hole,650+i*520));
        ...
        // ===== PET DO ELO · V36.10.0 · CENTRAL DOS PETS / UX =====
        ...
        const getPetCare = pet => ({affection:Math.max(0,Math.min(100,Number(pet?.care?.affection??70))),hunger:Math.max(0,Math.min(100,Number(pet?.care?.hunger??70))),energy:Math.max(0,Math.min(100,Number(pet?.care?.energy??70)))});
        ...
        const petMood = pet => {const care=getPetCare(pet),avg=(care.affection+care.hunger+care.energy)/3;if(care.hunger<30)return{emoji:'🍖',label:'com fome',tone:'hungry'};if(care.energy<28)return{emoji:'💤',label:'com sono',tone:'sleepy'};if(care.affection<32)return{emoji:'💗',label:'quer carinho',tone:'lonely'};if(avg>86)return{emoji:'✨',label:'radiante',tone:'happy'};return{emoji:'❤',label:'feliz',tone:'calm'}};
        const petRecordId = (pet,data=coupleData) => String(pet?.id || data?.petActiveId || `pet_${pet?.type||'pet'}_${pet?.hatchedAt||pet?.adoptedAt||'legacy'}`).replace(/[^a-zA-Z0-9_-]/g,'').slice(0,80);
        const activePetFrom = data => data?.pet?.status==='adopted' ? data.pet : null;
        const eggProposalFrom = data => data?.petEggProposal || (data?.pet?.status==='proposal' ? data.pet : null);
        const incubatorFrom = data => data?.petIncubator || (data?.pet?.status==='egg' ? data.pet : null);
        const petCollectionFrom = data => {const out={...(data?.petCollection||{})};const active=activePetFrom(data);if(active){const id=petRecordId(active,data);if(!out[id])out[id]={...active,id}}return out};
        const eggInventoryFrom = data => ({common:Number(data?.petEggInventory?.common||0),uncommon:Number(data?.petEggInventory?.uncommon||0),rare:Number(data?.petEggInventory?.rare||0),legendary:Number(data?.petEggInventory?.legendary||0)});
        const totalEggsInInventory = data => Object.values(eggInventoryFrom(data)).reduce((a,b)=>a+b,0);
        ...
        const clampFloatingPetPosition=(x,y,host=document.getElementById('elo-floating-pet-host'))=>{const w=host?.offsetWidth||76,h=host?.offsetHeight||90,pad=8,nav=74;return{x:Math.max(pad,Math.min(innerWidth-w-pad,x)),y:Math.max(pad+8,Math.min(innerHeight-h-nav-Math.max(0,parseInt(getComputedStyle(document.documentElement).getPropertyValue('--sat')||'0')),y))}};
        const saveFloatingPetPosition=host=>{if(!host)return;const r=host.getBoundingClientRect();localStorage.setItem(petFloatPositionKey(),JSON.stringify({x:(r.left+r.width/2)/innerWidth,y:(r.top+r.height/2)/innerHeight}))};
        const applyStoredFloatingPetPosition=host=>{if(!host)return;let pos=null;try{pos=JSON.parse(localStorage.getItem(petFloatPositionKey())||'null')}catch(_){}const w=host.offsetWidth||76,h=host.offsetHeight||90,raw=pos?{x:Number(pos.x)*innerWidth-w/2,y:Number(pos.y)*innerHeight-h/2}:{x:innerWidth-w-12,y:innerHeight-h-88},p=clampFloatingPetPosition(raw.x,raw.y,host);host.style.left=`${p.x}px`;host.style.top=`${p.y}px`;host.style.right='auto';host.style.bottom='auto'};
        const removeFloatingPetBackdrop=()=>document.getElementById('elo-floating-pet-backdrop')?.remove(); window.removeFloatingPetBackdrop=removeFloatingPetBackdrop;
        const positionFloatingPetMenu=shell=>{const menu=shell?.querySelector('.elo-floating-pet-menu'),host=document.getElementById('elo-floating-pet-host');if(!menu||!host)return;menu.style.visibility='hidden';menu.style.display='block';const mr=menu.getBoundingClientRect(),hr=host.getBoundingClientRect(),gap=8,pad=8;let left=hr.right+gap,top=hr.top;if(left+mr.width>innerWidth-pad)left=hr.left-mr.width-gap;if(left<pad)left=Math.max(pad,Math.min(innerWidth-mr.width-pad,hr.left+hr.width/2-mr.width/2));if(top+mr.height>innerHeight-pad)top=hr.bottom-mr.height;if(top<pad)top=pad;menu.style.left=`${Math.round(left)}px`;menu.style.top=`${Math.round(top)}px`;menu.style.right='auto';menu.style.bottom='auto';menu.style.visibility=''};
        const addFloatingPetBackdrop=()=>{removeFloatingPetBackdrop();const bd=document.createElement('div');bd.id='elo-floating-pet-backdrop';bd.setAttribute('aria-label','Fechar menu do Pet');const close=e=>{e.preventDefault();e.stopPropagation();closeFloatingPetMenu()};bd.addEventListener('pointerdown',close,{passive:false});bd.addEventListener('click',e=>{e.preventDefault();e.stopPropagation()});document.body.appendChild(bd)};
        const initFloatingPetDrag=host=>{if(!host||host.dataset.dragReady==='1')return;host.dataset.dragReady='1';host.addEventListener('pointerdown',e=>{const btn=e.target.closest('.elo-floating-pet,.elo-floating-egg');if(!btn||e.button>0)return;const r=host.getBoundingClientRect();floatingPetDragState={id:e.pointerId,startX:e.clientX,startY:e.clientY,left:r.left,top:r.top,moved:false};btn.setPointerCapture?.(e.pointerId);host.classList.add('is-grabbing');closeFloatingPetMenu()});host.addEventListener('pointermove',e=>{const d=floatingPetDragState;if(!d||d.id!==e.pointerId)return;const dx=e.clientX-d.startX,dy=e.clientY-d.startY;if(Math.hypot(dx,dy)>5)d.moved=true;if(!d.moved)return;e.preventDefault();const p=clampFloatingPetPosition(d.left+dx,d.top+dy,host);host.style.left=`${p.x}px`;host.style.top=`${p.y}px`;host.style.right='auto';host.style.bottom='auto'});const finish=e=>{const d=floatingPetDragState;if(!d||d.id!==e.pointerId)return;host.classList.remove('is-grabbing');if(d.moved){suppressFloatingPetClick=true;saveFloatingPetPosition(host);setTimeout(()=>suppressFloatingPetClick=false,140)}floatingPetDragState=null};host.addEventListener('pointerup',finish);host.addEventListener('pointercancel',finish)};
        window.addEventListener('resize',()=>{const h=document.getElementById('elo-floating-pet-host');if(h){applyStoredFloatingPetPosition(h);const shell=h.querySelector('[data-pet-menu="open"]');if(shell)requestAnimationFrame(()=>positionFloatingPetMenu(shell))}});
        const petCooldownRemaining=(pet,action)=>Math.max(0,({affection:45*60*1000,feed:2*60*60*1000,play:90*60*1000}[action]||0)-(Date.now()-Number(pet?.activity?.[action]?.at||0)));
        const petQuickAction=(pet,action,icon,label)=>{const remaining=petCooldownRemaining(pet,action),disabled=remaining>0,sub=disabled?`${Math.max(1,Math.ceil(remaining/60000))}m`:'agora';return`<button type="button" ${disabled?'disabled':''} onclick="careForPet('${action}',{floating:true})"><span>${icon}</span><b>${label}</b><small>${sub}</small></button>`};
        ...
        const eggCooldownRemaining=(egg,action)=>Math.max(0,(eggCareRules[action]?.cooldown||0)-(Date.now()-Number(egg?.eggActivity?.[action]?.at||0)));
        ...
        const rollPetFromEgg=eggId=>{const egg=ELO_EGG_CATALOG[eggId]||ELO_EGG_CATALOG.common,roll=Math.random()*100;let acc=0,rarity='common';for(const[key,pct]of Object.entries(egg.odds)){acc+=pct;if(roll<=acc){rarity=key;break}}const pool=Object.values(ELO_PET_CATALOG).filter(p=>p.rarityKey===rarity);return(pool[Math.floor(Math.random()*pool.length)]||ELO_PET_CATALOG.shiba).id};
        const renderFloatingEgg=egg=>{const progress=Math.max(0,Math.min(100,Number(egg?.hatchProgress||0))),item=ELO_EGG_CATALOG[egg.eggType]||ELO_EGG_CATALOG.common;return`<div class="elo-floating-pet-shell elo-floating-egg-shell" data-pet-menu="closed"><div class="elo-floating-pet-menu" aria-hidden="true"><div class="elo-floating-pet-mini-status"><span>🥚</span><div><b>${item.name}</b><small>${progress}% para chocar · cuidem juntos</small></div></div><div class="elo-floating-pet-actions egg-actions">${Object.entries(eggCareRules).map(([a,r])=>{const rem=eggCooldownRemaining(egg,a);return`<button ${rem?'disabled':''} onclick="careForEgg('${a}',{floating:true})"><span>${r.icon}</span><b>${r.label}</b><small>${rem?Math.max(1,Math.ceil(rem/60000))+'m':'agora'}</small></button>`}).join('')}</div><button class="elo-floating-pet-open" onclick="openPetCenter()">Abrir Central dos Pets <i class="ph-bold ph-arrow-up-right"></i></button></div><button type="button" class="elo-floating-egg ${eggCrackClass(progress)}" onclick="toggleFloatingPetMenu(event)" aria-label="Interagir com o ovo"><span class="elo-egg-aura"></span><span class="elo-egg-emoji">🥚</span><span class="elo-egg-crack">⌁</span><span class="elo-floating-pet-name">${progress}%</span></button></div>`};
        const updateFloatingPetCompanion=()=>{let host=document.getElementById('elo-floating-pet-host');const active=activePetFrom(coupleData),incubator=incubatorFrom(coupleData),proposal=eggProposalFrom(coupleData),inventory=eggInventoryFrom(coupleData);const appVisible=!!currentUser&&!!coupleData&&!document.getElementById('main-content')?.classList.contains('hidden');if(!appVisible){removeFloatingPetBackdrop();host?.remove();return}if(!host){host=document.createElement('div');host.id='elo-floating-pet-host';document.body.appendChild(host)}removeFloatingPetBackdrop();if(active){const item=petCatalogItem(active),care=getPetCare(active),mood=petMood(active),level=getPetLevel(active),eggBadge=incubator?`<span class="elo-floating-incubator-badge">🥚 ${Math.round(Number(incubator.hatchProgress||0))}%</span>`:'';host.innerHTML=`<div class="elo-floating-pet-shell is-${mood.tone}" data-pet-menu="closed"><div class="elo-floating-pet-menu" aria-hidden="true"><div class="elo-floating-pet-mini-status"><span>${mood.emoji}</span><div><b>${escapeHTML(petDisplayName(active))} · Nv. ${level}</b><small>${mood.label} · ${Math.round((care.affection+care.hunger+care.energy)/3)}% bem-estar</small></div></div><div class="elo-floating-pet-actions">${petQuickAction(active,'affection','🤍','Carinho')}${petQuickAction(active,'feed','🍖','Comida')}${petQuickAction(active,'play','🎾','Brincar')}</div><button class="elo-floating-pet-open" onclick="openPetCenter()">Central dos Pets <i class="ph-bold ph-arrow-up-right"></i></button></div><button type="button" class="elo-floating-pet" onclick="toggleFloatingPetMenu(event)" aria-label="Interagir com ${escapeHTML(petDisplayName(active))}"><span class="elo-floating-pet-bubble">${mood.emoji}</span>${eggBadge}<span class="elo-floating-pet-shadow"></span><span class="elo-floating-pet-emoji">${item.emoji}</span><span class="elo-floating-pet-name">${escapeHTML(petDisplayName(active))}</span></button></div>`;applyStoredFloatingPetPosition(host);initFloatingPetDrag(host);return}if(incubator){host.innerHTML=renderFloatingEgg(incubator);applyStoredFloatingPetPosition(host);initFloatingPetDrag(host);return}if(proposal){const egg=ELO_EGG_CATALOG[proposal.eggType]||ELO_EGG_CATALOG.common,waiting=proposal.proposedBy===currentUser?.uid;host.innerHTML=`<button class="elo-floating-pet-adopt is-pending" onclick="openPetCenter()"><span>${egg.emoji}</span><small>${waiting?'Aguardando':'Pedido!'}</small></button>`;applyStoredFloatingPetPosition(host);return}const eggs=Object.values(inventory).reduce((a,b)=>a+b,0);host.innerHTML=`<button class="elo-floating-pet-adopt" onclick="openPetCenter()"><span>🥚</span><small>${eggs?`${eggs} ovo${eggs>1?'s':''}`:'Pets'}</small></button>`;applyStoredFloatingPetPosition(host)};
        window.toggleFloatingPetMenu=event=>{event?.preventDefault?.();event?.stopPropagation?.();if(suppressFloatingPetClick)return;const shell=event?.currentTarget?.closest('.elo-floating-pet-shell');if(!shell)return;const open=shell.dataset.petMenu==='open';if(open){closeFloatingPetMenu();return}closeFloatingPetMenu();shell.dataset.petMenu='open';shell.querySelector('.elo-floating-pet-menu')?.setAttribute('aria-hidden','false');addFloatingPetBackdrop();requestAnimationFrame(()=>positionFloatingPetMenu(shell));event.currentTarget.classList.remove('is-reacting');void event.currentTarget.offsetWidth;event.currentTarget.classList.add('is-reacting')};
        window.closeFloatingPetMenu=()=>{document.querySelectorAll('.elo-floating-pet-shell[data-pet-menu="open"]').forEach(shell=>{shell.dataset.petMenu='closed';shell.querySelector('.elo-floating-pet-menu')?.setAttribute('aria-hidden','true')});removeFloatingPetBackdrop()};
        window.petHomeReact=event=>toggleFloatingPetMenu(event);
        window.pulseEloThread=event=>{const scene=document.querySelector('.elo-home-couple-scene');if(!scene)return;scene.classList.remove('elo-thread-reacting');void scene.offsetWidth;scene.classList.add('elo-thread-reacting');const rect=scene.getBoundingClientRect(),x=Math.max(18,Math.min(rect.width-18,(event?.clientX||rect.left+rect.width/2)-rect.left)),y=Math.max(18,Math.min(rect.height-18,(event?.clientY||rect.top+rect.height/2)-rect.top)),burst=document.createElement('span');burst.className='elo-thread-burst';burst.style.left=`${x}px`;burst.style.top=`${y}px`;burst.innerHTML='<i>♥</i><i>♥</i><i>♥</i><i>♥</i>';scene.appendChild(burst);setTimeout(()=>burst.remove(),950)};

        const petHubNav=()=>{const collection=petCollectionFrom(coupleData),inventory=eggInventoryFrom(coupleData),incubator=incubatorFrom(coupleData);return`<div class="elo-pet-hub-nav"><button onclick="openPetCollection()"><span>🐾</span><b>Coleção</b><small>${Object.keys(collection).length} Pet${Object.keys(collection).length===1?'':'s'}</small></button><button onclick="openEggInventory()"><span>🎒</span><b>Ovos</b><small>${Object.values(inventory).reduce((a,b)=>a+b,0)} guardado${Object.values(inventory).reduce((a,b)=>a+b,0)===1?'':'s'}</small></button><button onclick="openPetIncubator()"><span>🥚</span><b>Incubadora</b><small>${incubator?`${Math.round(Number(incubator.hatchProgress||0))}%`:'livre'}</small></button><button onclick="openEggShop()"><span>🛒</span><b>Loja de ovos</b><small>novos companheiros</small></button></div>`};
        window.openPetCenter=()=>{closeFloatingPetMenu();if(!getPetPartnerUid(coupleData))return openGenericModal(`<div class="elo-pet-center"><div class="elo-pet-hero"><span>🐾</span><div><p class="elo-kicker">Central dos Pets</p><h3>Um cantinho de vocês</h3><p>Conecte seu amor primeiro. Pets, ovos e incubadora pertencem ao casal.</p></div></div><button class="elo-primary-action" onclick="closeGenericModal()">Entendi</button></div>`);const proposal=eggProposalFrom(coupleData),active=activePetFrom(coupleData),incubator=incubatorFrom(coupleData);if(proposal)return openPetEggProposal();let activeHtml='';if(active){const item=petCatalogItem(active),care=getPetCare(active),level=getPetLevel(active),pending=active.pendingName,nameBlock=!active.name?(pending?`<div class="elo-pet-name-box"><p>${pending.proposedBy===currentUser.uid?'Você sugeriu':'Seu amor sugeriu'} <b>“${escapeHTML(pending.value)}”</b></p>${pending.proposedBy===currentUser.uid?'<small>Aguardando aprovação.</small>':'<button onclick="acceptPetName()">Aprovar nome ❤️</button>'}</div>`:`<div class="elo-pet-name-box"><p><b>Escolham o nome juntos</b></p><div><input id="elo-pet-name-input" maxlength="20" placeholder="Nome do Pet"><button onclick="proposePetName()">Sugerir</button></div></div>`):`<div class="elo-pet-name-box"><p><b>${escapeHTML(active.name)}</b> é o Pet ativo do Elo.</p>${pending?`<small>Renomeação sugerida: “${escapeHTML(pending.value)}” ${pending.proposedBy!==currentUser.uid?'<button onclick="acceptPetName()">Aprovar</button>':'· aguardando'}</small>`:`<button onclick="openPetRename()">Sugerir novo nome · ${ELO_PET_RENAME_PRICE} 🪙</button>`}</div>`;activeHtml=`<div class="elo-pet-profile"><div class="elo-pet-profile-emoji">${item.emoji}</div><div><span class="elo-pet-rarity">${item.rarity}</span><h3>${escapeHTML(petDisplayName(active))}</h3><p>${item.species} · Nível ${level}</p></div><b>${Number(active.xp||0)%100}/100 XP</b></div>${nameBlock}<div class="elo-pet-bars"><label><span>❤️ Carinho</span><b>${care.affection}%</b><i><em style="width:${care.affection}%"></em></i></label><label><span>🍖 Fome</span><b>${care.hunger}%</b><i><em style="width:${care.hunger}%"></em></i></label><label><span>⚡ Energia</span><b>${care.energy}%</b><i><em style="width:${care.energy}%"></em></i></label></div><div class="elo-pet-care-actions"><button onclick="careForPet('affection')"><span>🤍</span><b>Carinho</b><small>+ vínculo</small></button><button onclick="careForPet('feed')"><span>🍖</span><b>Alimentar</b><small>+ fome</small></button><button onclick="careForPet('play')"><span>🎾</span><b>Brincar</b><small>+ XP</small></button></div>`}else activeHtml=`<div class="elo-pet-empty-active"><span>🐾</span><div><b>Nenhum Pet ativo</b><small>${incubator?'Seu ovo está crescendo na incubadora.':'Abra um ovo ou escolha um Pet da coleção.'}</small></div></div>`;openGenericModal(`<div class="elo-pet-center"><button type="button" class="elo-pet-center-close" onclick="closeGenericModal()" aria-label="Fechar Central dos Pets"><i class="ph-bold ph-x"></i></button><div class="elo-pet-hero"><span>🏡</span><div><p class="elo-kicker">Central dos Pets</p><h3>${active?'Seu companheiro ativo':'O cantinho dos Pets'}</h3><p>Gerencie coleção, ovos e incubadora sem perder o Pet que já está com vocês.</p></div></div>${activeHtml}${petHubNav()}<p class="elo-pet-footnote">O Pet flutuante pode ser arrastado. Toque nele para abrir ou fechar o painel rápido.</p></div>`)};
        window.openPetCollection=()=>{const collection=petCollectionFrom(coupleData),active=activePetFrom(coupleData),activeId=active?petRecordId(active,coupleData):null,rows=Object.entries(collection).map(([id,pet])=>{const item=petCatalogItem(pet),on=id===activeId||id===coupleData?.petActiveId;return`<article class="elo-pet-collection-card ${on?'is-active':''}"><div class="elo-pet-collection-emoji">${item.emoji}</div><div><span>${item.rarity}</span><b>${escapeHTML(petDisplayName(pet))}</b><small>${item.species} · Nv. ${getPetLevel(pet)}</small></div>${on?'<em>ATIVO</em>':`<button onclick="setActivePet('${id}')">Tornar ativo</button>`}</article>`}).join('')||'<div class="elo-pet-empty-active"><span>🐾</span><div><b>A coleção ainda está vazia</b><small>Choque um ovo para encontrar o primeiro Pet.</small></div></div>';openGenericModal(`<div class="elo-pet-center"><div class="elo-pet-hero"><span>🐾</span><div><p class="elo-kicker">Coleção do casal</p><h3>Todos os Pets</h3><p>Vocês podem ter vários companheiros e escolher qual aparece flutuando no app.</p></div></div><div class="elo-pet-collection-grid">${rows}</div><button class="elo-secondary-action" onclick="closeGenericModal();setTimeout(openPetCenter,70)">Voltar à Central</button></div>`)};
        window.setActivePet=async id=>{try{const ref=doc(db,'relationships',coupleId);await runTransaction(db,async tx=>{const snap=await tx.get(ref),data=snap.data(),collection={...(data.petCollection||{})},current=activePetFrom(data);if(current){const cid=petRecordId(current,data);collection[cid]={...current,id:cid}}const target=collection[id];if(!target)throw new Error('missing');tx.update(ref,{pet:{...target,id,status:'adopted'},petActiveId:id,petCollection:collection})});closeGenericModal();setTimeout(openPetCollection,170)}catch(e){console.error(e);showToast('Não foi possível trocar o Pet ativo.','error')}};
        window.openEggInventory=()=>{const inv=eggInventoryFrom(coupleData),incubator=incubatorFrom(coupleData),cards=Object.values(ELO_EGG_CATALOG).map(egg=>{const count=inv[egg.id]||0;return`<article class="elo-egg-inventory-card is-${egg.accent}"><span>🥚</span><div><b>${egg.name}</b><small>${count} no inventário</small><em>${eggOddsText(egg)}</em></div><button ${!count||incubator?'disabled':''} onclick="startEggIncubation('${egg.id}')">${incubator?'Incubadora ocupada':count?'Incubar':'Sem ovos'}</button></article>`}).join('');openGenericModal(`<div class="elo-pet-center"><div class="elo-pet-hero"><span>🎒</span><div><p class="elo-kicker">Inventário de ovos</p><h3>Escolham o próximo ovo</h3><p>Comprar um ovo agora guarda ele aqui. A incubadora usa um ovo por vez.</p></div></div><div class="elo-egg-inventory-list">${cards}</div><button class="elo-primary-action" onclick="closeGenericModal();setTimeout(openEggShop,70)">Comprar mais ovos</button><button class="elo-secondary-action" onclick="closeGenericModal();setTimeout(openPetCenter,70)">Voltar à Central</button></div>`)};
        window.openEggShop=()=>{const proposal=eggProposalFrom(coupleData);if(proposal)return openPetEggProposal();const cards=Object.values(ELO_EGG_CATALOG).map(egg=>`<article class="elo-egg-shop-card is-${egg.accent}"><div class="elo-egg-shop-visual"><span class="elo-shop-egg">🥚</span><small>${egg.name}</small></div><div><p>${egg.description}</p><b>${egg.price.toLocaleString('pt-BR')} 🪙</b><small class="elo-egg-odds">${eggOddsText(egg)}</small></div><div class="elo-pet-adopt-actions"><button onclick="proposeEggAdoption('${egg.id}','split')">Dividir 50/50</button><button class="is-primary" onclick="proposeEggAdoption('${egg.id}','full')">Eu pago tudo</button></div></article>`).join('');openGenericModal(`<div class="elo-pet-center"><div class="elo-pet-hero"><span>🛒</span><div><p class="elo-kicker">Loja de ovos</p><h3>Adicionar ao inventário</h3><p>O ovo comprado não substitui o Pet ativo. Depois da aprovação, ele vai para o inventário do casal.</p></div></div><div class="elo-egg-shop-grid">${cards}</div><button class="elo-secondary-action" onclick="closeGenericModal();setTimeout(openPetCenter,70)">Voltar à Central</button></div>`)};
        window.openPetEggProposal=()=>{const pet=eggProposalFrom(coupleData);if(!pet)return openEggShop();const egg=ELO_EGG_CATALOG[pet.eggType]||ELO_EGG_CATALOG.common,mine=pet.proposedBy===currentUser.uid,minePay=Number(pet.contributions?.[currentUser.uid]||0),otherUid=getPetPartnerUid(coupleData),otherPay=Number(pet.contributions?.[otherUid]||0);openGenericModal(`<div class="elo-pet-center"><div class="elo-pet-hero"><span>🥚</span><div><p class="elo-kicker">Pedido de ovo</p><h3>${egg.name}</h3><p>${mine?'Sua proposta está esperando seu amor.':'Seu amor quer adicionar este ovo ao inventário de vocês.'}</p></div></div><div class="elo-pet-proposal-summary"><span><small>Sua parte</small><b>${minePay.toLocaleString('pt-BR')} 🪙</b></span><span><small>Parte do amor</small><b>${otherPay.toLocaleString('pt-BR')} 🪙</b></span></div><small class="elo-egg-odds">${eggOddsText(egg)}</small>${mine?`<button class="elo-secondary-action" onclick="cancelPetProposal()">Cancelar proposta</button>`:`<div class="elo-pet-decision"><button class="elo-secondary-action" onclick="rejectPetProposal()">Agora não</button><button class="elo-primary-action" onclick="acceptEggAdoption()">Comprar o ovo ❤️</button></div>`}</div>`)};
        window.openPetIncubator=()=>{const egg=incubatorFrom(coupleData);if(!egg)return openGenericModal(`<div class="elo-pet-center"><div class="elo-pet-hero"><span>🥚</span><div><p class="elo-kicker">Incubadora</p><h3>Está livre</h3><p>Escolha um ovo do inventário para começar uma nova incubação.</p></div></div><button class="elo-primary-action" onclick="closeGenericModal();setTimeout(openEggInventory,70)">Escolher um ovo</button><button class="elo-secondary-action" onclick="closeGenericModal();setTimeout(openPetCenter,70)">Voltar à Central</button></div>`);const item=ELO_EGG_CATALOG[egg.eggType]||ELO_EGG_CATALOG.common,progress=Math.max(0,Math.min(100,Number(egg.hatchProgress||0))),actions=Object.entries(eggCareRules).map(([a,r])=>{const rem=eggCooldownRemaining(egg,a);return`<button ${rem?'disabled':''} onclick="careForEgg('${a}')"><span>${r.icon}</span><b>${r.label}</b><small>${rem?Math.max(1,Math.ceil(rem/60000))+' min':`+${r.progress}%`}</small></button>`}).join('');openGenericModal(`<div class="elo-pet-center elo-incubator"><div class="elo-pet-hero"><span>🥚</span><div><p class="elo-kicker">Incubadora</p><h3>${item.name}</h3><p>Cada cuidado aproxima a revelação. Os dois podem ajudar.</p></div></div><div class="elo-incubator-stage ${eggCrackClass(progress)}"><span class="elo-incubator-glow"></span><span class="elo-incubator-egg">🥚</span><span class="elo-incubator-crack">⌁</span><b>${progress}%</b><i><em style="width:${progress}%"></em></i></div><div class="elo-pet-care-actions elo-egg-care-actions">${actions}</div><small class="elo-egg-odds">Possibilidades: ${eggOddsText(item)}</small>${progress>=100?'<button class="elo-primary-action elo-hatch-button" onclick="hatchPetEgg()">Chocar agora ✨</button>':''}<button class="elo-secondary-action" onclick="closeGenericModal();setTimeout(openPetCenter,70)">Voltar à Central</button></div>`)};
        window.proposeEggAdoption=async(eggType,mode='split')=>{const egg=ELO_EGG_CATALOG[eggType];if(!egg||!coupleId||!currentUser)return;const otherUid=getPetPartnerUid(coupleData);if(!otherUid)return showToast('Conecte seu amor antes de comprar.','error');if(eggProposalFrom(coupleData))return showToast('Já existe uma proposta de ovo aguardando.','error');const mine=mode==='full'?egg.price:Math.ceil(egg.price/2),other=mode==='full'?0:egg.price-mine;if(getSpendableCoins(coupleData,currentUser.uid)<mine)return showToast(`Você precisa de ${mine} Elo Coins para sua parte.`,'error');try{await updateDoc(doc(db,'relationships',coupleId),{petEggProposal:{status:'proposal',kind:'egg',eggType:egg.id,price:egg.price,proposedBy:currentUser.uid,mode,contributions:{[currentUser.uid]:mine,[otherUid]:other},requestedAt:Date.now()}});closeGenericModal();showToast('Proposta do ovo enviada.','success')}catch(e){console.error(e);showToast('Não foi possível enviar a proposta.','error')}};
        window.proposePetAdoption=window.proposeEggAdoption;
        window.cancelPetProposal=async()=>{try{const legacy=coupleData?.pet?.status==='proposal';await updateDoc(doc(db,'relationships',coupleId),legacy?{pet:deleteField()}:{petEggProposal:deleteField()});closeGenericModal()}catch(e){showToast('Não foi possível cancelar.','error')}};
        window.rejectPetProposal=async()=>{try{const legacy=coupleData?.pet?.status==='proposal';await updateDoc(doc(db,'relationships',coupleId),legacy?{pet:deleteField()}:{petEggProposal:deleteField()});closeGenericModal()}catch(e){showToast('Não foi possível responder agora.','error')}};
        window.acceptEggAdoption=async()=>{try{const ref=doc(db,'relationships',coupleId);await runTransaction(db,async tx=>{const snap=await tx.get(ref);if(!snap.exists())throw new Error('elo');const data=snap.data(),proposal=data.petEggProposal||(data.pet?.status==='proposal'?data.pet:null);if(!proposal||proposal.proposedBy===currentUser.uid)throw new Error('proposal');const updates={};for(const[uid,amountRaw]of Object.entries(proposal.contributions||{})){const amount=Number(amountRaw||0),balance=Number(data?.users?.[uid]?.coins||0);if(balance<amount)throw new Error('coins');if(amount>0)updates[`users.${uid}.coins`]=balance-amount}updates[`petEggInventory.${proposal.eggType}`]=Number(data?.petEggInventory?.[proposal.eggType]||0)+1;if(data.petEggProposal)updates.petEggProposal=deleteField();else if(data.pet?.status==='proposal')updates.pet=deleteField();tx.update(ref,updates)});closeGenericModal();showToast('🥚 Ovo adicionado ao inventário!','success');setTimeout(openEggInventory,260)}catch(e){console.error(e);showToast(e?.message==='coins'?'As Elo Coins mudaram. Confiram o saldo.':'Não foi possível comprar o ovo.','error')}};
        window.acceptPetAdoption=window.acceptEggAdoption;
        window.startEggIncubation=async eggType=>{const item=ELO_EGG_CATALOG[eggType];if(!item)return;try{const ref=doc(db,'relationships',coupleId);await runTransaction(db,async tx=>{const snap=await tx.get(ref),data=snap.data();if(incubatorFrom(data))throw new Error('busy');const count=Number(data?.petEggInventory?.[eggType]||0);if(count<1)throw new Error('empty');tx.update(ref,{[`petEggInventory.${eggType}`]:count-1,petIncubator:{status:'egg',eggType,price:item.price,startedAt:Date.now(),hatchProgress:0,eggActivity:{}}})});closeGenericModal();setTimeout(openPetIncubator,230)}catch(e){showToast(e?.message==='busy'?'A incubadora já está ocupada.':'Esse ovo não está mais no inventário.','error')}};
        window.careForEgg=async(action,opts={})=>{const rule=eggCareRules[action];if(!rule)return;try{const ref=doc(db,'relationships',coupleId);await runTransaction(db,async tx=>{const snap=await tx.get(ref),data=snap.data(),egg=data?.petIncubator||(data?.pet?.status==='egg'?data.pet:null);if(!egg)throw new Error('egg');const now=Date.now(),last=Number(egg?.eggActivity?.[action]?.at||0),remaining=rule.cooldown-(now-last);if(remaining>0){const err=new Error('cooldown');err.remaining=remaining;throw err}const next=Math.min(100,Number(egg.hatchProgress||0)+rule.progress),prefix=data.petIncubator?'petIncubator':'pet';tx.update(ref,{[`${prefix}.hatchProgress`]:next,[`${prefix}.eggActivity.${action}`]:{at:now,by:currentUser.uid}})});if(opts.floating){document.querySelector('.elo-floating-egg')?.classList.add('is-reacting');setTimeout(updateFloatingPetCompanion,160)}else{closeGenericModal();setTimeout(openPetIncubator,220)}}catch(e){if(e?.message==='cooldown'){const min=Math.max(1,Math.ceil(Number(e.remaining||0)/60000));if(opts.floating){const n=document.querySelector('.elo-floating-pet-name');if(n){n.textContent=`${min}m`;setTimeout(updateFloatingPetCompanion,900)}return}return showToast(`Esse cuidado volta em ${min} min.`,'error')}console.error(e);showToast('Não foi possível cuidar do ovo agora.','error')}};
        window.hatchPetEgg=async()=>{try{const ref=doc(db,'relationships',coupleId);let revealed=null,newId=`pet_${Date.now()}_${Math.random().toString(36).slice(2,7)}`;await runTransaction(db,async tx=>{const snap=await tx.get(ref),data=snap.data(),egg=data?.petIncubator||(data?.pet?.status==='egg'?data.pet:null);if(!egg||Number(egg.hatchProgress||0)<100)throw new Error('not-ready');const type=rollPetFromEgg(egg.eggType);revealed=ELO_PET_CATALOG[type];const collection={...(data.petCollection||{})},oldActive=activePetFrom(data);if(oldActive){const oldId=petRecordId(oldActive,data);collection[oldId]={...oldActive,id:oldId}}const newPet={id:newId,status:'adopted',type,eggType:egg.eggType,price:egg.price||0,hatchedAt:Date.now(),adoptedAt:Date.now(),adoptedBy:[currentUser.uid],name:'',pendingName:null,xp:0,care:{affection:78,hunger:82,energy:86,lastCareAt:Date.now()},activity:{}};collection[newId]=newPet;const updates={pet:newPet,petActiveId:newId,petCollection:collection};if(data.petIncubator)updates.petIncubator=deleteField();tx.update(ref,updates)});closeGenericModal();openGenericModal(`<div class="elo-pet-center elo-hatch-reveal"><p class="elo-kicker">O ovo chocou!</p><div class="elo-hatch-light"></div><div class="elo-hatch-pet">${revealed?.emoji||'🐾'}</div><span class="elo-pet-rarity">${revealed?.rarity||''}</span><h3>${revealed?.species||'Novo Pet'}</h3><p>${revealed?.description||'Um novo companheiro chegou ao Elo.'}</p><button class="elo-primary-action" onclick="closeGenericModal();setTimeout(openPetCollection,100)">Ver nossa coleção ❤️</button></div>`)}catch(e){console.error(e);showToast('O ovo ainda não está pronto para chocar.','error')}};
        window.proposePetName=async(forcedName='')=>{const pet=activePetFrom(coupleData);if(!pet)return;const input=document.getElementById('elo-pet-name-input'),value=String(forcedName||input?.value||'').trim().slice(0,20);if(value.length<2)return showToast('Escolha um nome com pelo menos 2 caracteres.','error');const cost=pet.name?ELO_PET_RENAME_PRICE:0;if(cost&&getSpendableCoins(coupleData,currentUser.uid)<cost)return showToast(`Você precisa de ${cost} Elo Coins para propor a renomeação.`,'error');try{await updateDoc(doc(db,'relationships',coupleId),{'pet.pendingName':{value,proposedBy:currentUser.uid,cost,createdAt:Date.now()}});closeGenericModal()}catch(e){showToast('Não foi possível sugerir o nome.','error')}};
        ...
        window.acceptPetName=async()=>{try{const ref=doc(db,'relationships',coupleId);await runTransaction(db,async tx=>{const snap=await tx.get(ref),data=snap.data(),pet=activePetFrom(data),pending=pet?.pendingName;if(!pending||pending.proposedBy===currentUser.uid)throw new Error('name');const cost=Number(pending.cost||0),payer=pending.proposedBy,balance=Number(data?.users?.[payer]?.coins||0);if(cost>balance)throw new Error('coins');const updated={...pet,name:pending.value,pendingName:null,renamedAt:Date.now()},id=petRecordId(updated,data),collection={...(data.petCollection||{}),[id]:{...updated,id}},updates={pet:updated,petActiveId:id,petCollection:collection};if(cost>0)updates[`users.${payer}.coins`]=balance-cost;tx.update(ref,updates)});closeGenericModal();setTimeout(openPetCenter,200)}catch(e){showToast(e?.message==='coins'?'Quem sugeriu o nome não tem mais Coins suficientes.':'Não foi possível aprovar o nome.','error')}};
        window.careForPet=async(action,opts={})=>{const rules={affection:{cooldown:45*60*1000,xp:8},feed:{cooldown:2*60*60*1000,xp:10},play:{cooldown:90*60*1000,xp:16}},rule=rules[action];if(!rule)return;try{const ref=doc(db,'relationships',coupleId);await runTransaction(db,async tx=>{const snap=await tx.get(ref),data=snap.data(),pet=activePetFrom(data);if(!pet)throw new Error('pet');const last=Number(pet?.activity?.[action]?.at||0),now=Date.now(),remaining=rule.cooldown-(now-last);if(remaining>0){const err=new Error('cooldown');err.remaining=remaining;throw err}const care=getPetCare(pet),updated={...pet,xp:Number(pet.xp||0)+rule.xp,activity:{...(pet.activity||{}),[action]:{at:now,by:currentUser.uid}},care:{...(pet.care||{}),lastCareAt:now}};if(action==='affection')updated.care.affection=Math.min(100,care.affection+14);if(action==='feed'){updated.care.hunger=Math.min(100,care.hunger+18);updated.care.energy=Math.min(100,care.energy+4)}if(action==='play'){updated.care.affection=Math.min(100,care.affection+8);updated.care.energy=Math.max(0,care.energy-8)}const id=petRecordId(updated,data),collection={...(data.petCollection||{}),[id]:{...updated,id}};tx.update(ref,{pet:{...updated,id},petActiveId:id,petCollection:collection})});if(opts.floating){document.querySelector('.elo-floating-pet')?.classList.add('is-reacting');setTimeout(updateFloatingPetCompanion,120)}else{closeGenericModal();setTimeout(openPetCenter,180)}}catch(e){if(e?.message==='cooldown'){const min=Math.max(1,Math.ceil(Number(e.remaining||0)/60000));if(opts.floating){const bubble=document.querySelector('.elo-floating-pet-bubble');if(bubble){bubble.textContent=`${min}m`;setTimeout(updateFloatingPetCompanion,900)}return}return showToast(`Seu Pet já recebeu isso. Tente de novo em ${min} min.`,'error')}console.error(e);showToast('Não foi possível cuidar do Pet agora.','error')}};
...
const centerActiveStoreCategory = (smooth = true) => {
        ...
        window.openGenericModal = html => { window.closeFloatingPetMenu?.(); document.getElementById('elo-floating-pet-host')?.classList.add('is-behind-modal'); let m=document.getElementById('generic-modal'); if(!m){m=document.createElement('div');m.id='generic-modal';m.className='elo-modal-backdrop fixed inset-0 z-[100] bg-black/70 flex items-end sm:items-center justify-center sm:p-4';m.addEventListener('click',e=>{if(e.target===m)closeGenericModal();});document.body.appendChild(m);} delete m.dataset.eloModal; m.innerHTML=`<div class="elo-bottom-sheet w-full sm:max-w-md max-h-[88dvh] overflow-y-auto bg-slate-950 border border-slate-800 rounded-t-[2rem] sm:rounded-3xl p-5 shadow-2xl">${html}</div>`; };
        window.closeGenericModal=()=>{document.getElementById('generic-modal')?.remove();document.getElementById('elo-floating-pet-host')?.classList.remove('is-behind-modal');window.removeFloatingPetBackdrop?.()};
